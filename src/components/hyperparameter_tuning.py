...
def get_hyperparameters_flags(default_flags, 
                              hyperparameters_file, 
                              search_method, 
                              max_iters=None, 
                              seed=None):
    # type: (dict[str, list], str, str, int, int) -> list[dict[str, list[str]]]
    methods = {'gridsearch': get_grid_flags, 
               'randomsearch': get_random_flags}
    extra_parameters = {'seed': seed, 
                        'max_iters': max_iters} \
                        if search_method == 'randomsearch' \
                        else {}
    return methods[search_method](default_flags, 
                                  hyperparameters_file, 
                                  **extra_parameters)
...
def get_random_flags(default_flags, hyperparameters_file, max_iters, seed=None):
    ...
    random_instance.seed(seed) # random.seed is not enough to ensure reproducibility

    distribution_functions = {
        "int_truncnorm": lambda *args, **kwargs: \
            stats.truncnorm.rvs(*args, **kwargs).round().astype(int), 
    ...
    for hyperparameter_name, hyperparameter_info in hyperparameters.items():
        ...
        distribution_extra_params = {'size': max_iters, 
                                     'random_state': seed} \
                                    if hyperparameter_distribution != 'multinoulli' \
                                    else {'k': max_iters}
        distribution_extra_params = {**distribution_extra_params, 
                                     **hyperparameter_distribution_kwargs}
        random_values = distribution_function(*hyperparameter_distribution_args, 
                                              **distribution_extra_params)
        random_values = list(map(lambda p: [str(p)], 
                                 random_values)) # Convert all values to lists of strings
...
def get_grid_flags(default_flags, grid_file):
    ...
    for product in grid_cartesian_product:
        ...
        current_default_flags['model'] = [rename_model_file(default_model_name, 
                                                            product_and_param_names)]
        current_default_flags = {**current_default_flags, 
                                 **product_and_param_names}
